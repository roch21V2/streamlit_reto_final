...
selected_table = st.selectbox("Select a table to view", options=table_names)

if selected_table == "ia_ocupaciones_gigante_limpia2":
    df_ocupaciones = get_data(
        constants.VIEWS_NAMES[1],
        date_cols=["fecha_hoy"],
        float_cols=["ing_hab", "trevpec"],
    )
    ...
    with col1:
        lower_date = st.date_input(
            "Select the start date", value=df_ocupaciones["fecha_hoy"].min().date()
        )
        upper_date = st.date_input(
            "Select the end date", value=df_ocupaciones["fecha_hoy"].max().date()
        )
    ...
    with col2:
        type_room = st.multiselect(
            "Select the type of room",
            options=unique_types_room,
            default=unique_types_room[0],
        )

    with col3:
        state = st.multiselect(
            "Select the state of the reservation",
            options=unique_states,
            default=unique_states[0],
        )
    ...
    total_revenue_person = filtered_df["trevpec"].mean()
    ...
    with col1:
        st.metric(label="Total Number of Records", value=f"{total_rows:,}")
    ...
    trevpec_df = filtered_df.groupby("fecha_hoy").agg({"trevpec": "mean"}).reset_index()
    trevpec_fig = px.line(
        trevpec_df, x="fecha_hoy", y="trevpec", title="trevpec Over Time"
    )
    ...
    revenue_fig = px.line(
        revenue, x="fecha_hoy", y="ing_hab", title="Total Revenue Per Day"
    )
    ...
    room_count = (
        filtered_df.groupby(["fecha_hoy", "id_tipo_habitacion"])
        .size()
        .reset_index(name="count")
    )
    room_count_fig = px.bar(
        room_count,
        x="fecha_hoy",
        y="count",
        color="id_tipo_habitacion",
        title="Count of Type of Room Per Day",
    )
    ...
    df_empresa = get_data(
        constants.TABLE_NAMES[1],
        date_cols=["fecha_hoy"],
        float_cols=["ing_hab", "trevpec", "num_men", "num_adu", "cto_noc"],
    )
    ...
    with col1:
        lower_date = st.date_input(
            "Select the start date", value=df_empresa["fecha_hoy"].min().date()
        )
        upper_date = st.date_input(
            "Select the end date", value=df_empresa["fecha_hoy"].max().date()
        )
    ...
    with col2:
        company = st.multiselect(
            "Select the company", options=unique_companies, default=unique_companies[0]
        )
    ...
    total_trevpec = filtered_df["trevpec"].mean()
    ...
    with col1:
        st.metric(
            label="Total Number of Minors that Stayed", value=f"{int(total_num_men):,}"
        )
    with col2:
        st.metric(
            label="Total Number of Adults that Stayed", value=f"{int(total_num_adu):,}"
        )
    with col3:
        st.metric(
            label="Total Revenue Per Person",
    ...
    # Making a time series plot group by date
    trevpec_df = (
        filtered_df.groupby(["fecha_hoy", "id_empresa"])
        .agg({"trevpec": "mean"})
        .reset_index()
    )
    trevpec_fig = px.line(
        ...
        y="trevpec",
        color="id_empresa",
        title="trevpec Over Time By Company",
    )
    st.plotly_chart(trevpec_fig, use_container_width=True)
    ...
    ing_hab_fig = px.line(
        ...
        y="ing_hab",
        color="id_empresa",
        title="Total Revenue Ing_hab Over Time By Company",
    )
    st.plotly_chart(ing_hab_fig, use_container_width=True)

    cto_noc_df = (
        filtered_df.groupby(["fecha_hoy", "id_empresa"], as_index=False).agg(
            {"cto_noc": "mean"}
        )  # or 'sum' if more appropriate
    )
    ...
    df_entidad = get_data(
        constants.VIEWS_NAMES[0],
        date_cols=["fecha_hoy"],
        float_cols=["ing_hab", "trevpec", "num_men", "num_adu", "tasa_ocupacion"],
    )
    ...
    with col1:
        lower_date = st.date_input(
            "Select the start date", value=df_entidad["fecha_hoy"].min().date()
        )
        upper_date = st.date_input(
            "Select the end date", value=df_entidad["fecha_hoy"].max().date()
        )
    ...
    with col2:
        state = st.multiselect(
            "Select the state", options=unique_states, default=unique_states[0]
        )
    ...
    with col1:
        st.metric(
            label="Mean Occupation Rate (%)", value=f"{mean_ocupation_rate:,.2f} %"
        )
    with col2:
        st.metric(
    ...
    trevpec_df = (
        filtered_df.groupby(["fecha_hoy", "estado_nombre"])
        .agg({"trevpec": "mean"})
        .reset_index()
    )
    trevpec_fig = px.line(
        ...
        y="trevpec",
        color="estado_nombre",
        title="trevpec Over Time By State",
    )
    st.plotly_chart(trevpec_fig, use_container_width=True)
    ...
    ing_hab_fig = px.line(
        ...
        y="ing_hab",
        color="estado_nombre",
        title="Total Revenue Ing_hab Over Time By State",
    )
    st.plotly_chart(ing_hab_fig, use_container_width=True)

    tasa_ocupacion_df = (
        filtered_df.groupby(["fecha_hoy", "estado_nombre"])
        .agg({"tasa_ocupacion": "mean"})
        .reset_index()
    )
    tasa_ocupacion_fig = px.line(
        tasa_ocupacion_df,
        x="fecha_hoy",
        y="tasa_ocupacion",
        color="estado_nombre",
        title="Mean Occupation Rate Over Time by State",
    )
    ...
    df_room_type = get_data(
        constants.TABLE_NAMES[3],
        date_cols=["fecha_ocupacion"],
        float_cols=["tasa_ocupacion", "h_num_noc", "h_num_per"],
    )
    ...
    with col1:
        lower_date = st.date_input(
            "Select the start date", value=df_room_type["fecha_ocupacion"].min().date()
        )
        upper_date = st.date_input(
            "Select the end date", value=df_room_type["fecha_ocupacion"].max().date()
        )
    ...
    with col2:
        st.metric(
            label="Mean Number of Nights per Reservation", value=f"{h_num_noc:,.2f}"
        )
    with col3:
        st.metric(
    ...
    tasa_ocupacion_df = (
        filtered_df.groupby(["fecha_ocupacion", "id_tipo_habitacion"])
        .agg({"tasa_ocupacion": "mean"})
        .reset_index()
    )
    tasa_ocupacion_fig = px.line(
        ...
        y="tasa_ocupacion",
        color="id_tipo_habitacion",
        title="Mean Occupation Rate Over Time By Room Type",
    )
    st.plotly_chart(tasa_ocupacion_fig, use_container_width=True)
    ...
    h_num_noc_fig = px.line(
        h_num_noc_df,
        x="fecha_ocupacion",
        y="h_num_noc",
        color="id_tipo_habitacion",
        title="Mean Number of Nights per Reservation Over Time By Room Type",
    )
    ...
    h_num_per_fig = px.line(
        h_num_per_df,
        x="fecha_ocupacion",
        y="h_num_per",
        color="id_tipo_habitacion",
        title="Mean Number of People per Reservation Over Time By Room Type",
    )
